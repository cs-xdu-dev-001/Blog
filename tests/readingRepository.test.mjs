...
const hidden = repo.create({
  title: '暂不公开的书',
  author: '测试作者',
  status: 'planned',
  published: false,
});
assert.equal(hidden.published, 0);
assert.equal(repo.list().items.some((book) => book.id === hidden.id), true);
assert.equal(repo.list({ publishedOnly: true }).items.some((book) => book.id === hidden.id), false);

const republished = repo.update(hidden.id, { published: true });
assert.equal(republished.published, 1);
assert.equal(repo.list({ publishedOnly: true }).items.some((book) => book.id === hidden.id), true);
