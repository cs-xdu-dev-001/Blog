...
test('public URLs always use the production blog origin', () => {
  assert.equal(SITE_ORIGIN, 'https://blog.kards.asia');
  assert.equal(absoluteSiteUrl('/posts/hello?preview=1#top'), 'https://blog.kards.asia/posts/hello?preview=1#top');
  assert.equal(absoluteSiteUrl('uploads/cover.webp'), 'https://blog.kards.asia/uploads/cover.webp');
...
test('RSS contains published note metadata and escapes XML', () => {
  ...
  assert.match(xml, /<atom:link href="https:\/\/blog\.kards\.asia\/rss\.xml" rel="self" type="application\/rss\+xml" \/>/);
  assert.match(xml, /<link>https:\/\/blog\.kards\.asia\/posts\/rss-note<\/link>/);
  assert.match(xml, /<guid isPermaLink="true">https:\/\/blog\.kards\.asia\/posts\/rss-note<\/guid>/);
...
test('sitemap serializes public entries with optional update dates', () => {
  ...
  assert.match(xml, /^<\?xml version="1\.0" encoding="UTF-8"\?>/);
  assert.match(xml, /<loc>https:\/\/blog\.kards\.asia\/<\/loc>/);
  assert.match(xml, /<loc>https:\/\/blog\.kards\.asia\/posts\/a&amp;b<\/loc>/);
...
test('public layouts and XML routes expose complete publication metadata', () => {
  ...
  assert.match(config, /site:\s*SITE_ORIGIN/);
